...
view_w="${GPU_DEMO_VIEW_W:-900}"
view_h="${GPU_DEMO_VIEW_H:-600}"

set +e
out="$(/system/bin/sh "$ctl_path" demo aidl "$bridge_service" set-view "$view_w" "$view_h" 2>&1)"
rc=$?
set -e
printf '%s\n' "$out"
