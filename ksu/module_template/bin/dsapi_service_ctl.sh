...
ui_start() {
  ...
  if [ "$ui_state" = "running" ] && [ "$ui_visible" != "1" ]; then
    echo "ksu_dsapi_error=ui_not_foreground package=$ui_pkg component=$ui_comp focus=0 log=$MANAGER_HOST_LOG_FILE"
    return 2
  fi

  echo "ksu_dsapi_ui=started state=starting pid=- mode=parasitic_host package=$ui_pkg component=$ui_comp refresh_ms=$refresh_ms"
  return 0
}
...
module_cmd() {
  ...
  case "$sub" in
    zip-list)
      dsapi_module_zip_entries | while IFS='|' read -r zip_name zip_path
      do
        printf 'module_zip_row=%s|%s\n' "$zip_name" "$zip_path"
      done
      ;;
    list)
    ...
    install-builtin)
      zip_name="${2:-}"
      [ -n "$zip_name" ] || { echo "ksu_dsapi_error=module_zip_name_missing"; return 2; }
      case "$zip_name" in
        *.zip) zip_path="$MODROOT/module_zips/$zip_name" ;;
        *) zip_path="$MODROOT/module_zips/$zip_name.zip" ;;
      esac
      [ -f "$zip_path" ] || { echo "ksu_dsapi_error=module_zip_not_found path=$zip_path"; return 2; }
      mod_id="$(dsapi_module_install_zip "$zip_path")" || {
        dsapi_last_error_set "module.install" "install_failed" "module_install_failed" "path=$zip_path"
        echo "ksu_dsapi_error=module_install_failed path=$zip_path"
        return 2
    ...
    scope-clear)
      ...
      ;;
    *)
      echo "usage: dsapi_service_ctl.sh module zip-list|list|install-zip <zip>|install-builtin <name>|start <id>|stop <id>|reload <id>|reload-all|disable <id>|enable <id>|remove <id>|status <id>|detail <id>|action-list <id>|action-run <id> <action>|env-list <id>|env-set <id> <key> <value>|env-unset <id> <key>|scope-list [id]|scope-set <id> <package|*> <user|-1> <allow|deny>|scope-clear <id>"
      return 1
      ;;
  esac
}
...
usage:
  ...
  dsapi_service_ctl.sh [--v2] module zip-list|list|install-zip <zip>|install-builtin <name>|start <id>|stop <id>|reload <id>|reload-all|disable <id>|enable <id>|remove <id>|status <id>|detail <id>|action-list <id>|action-run <id> <action>|env-list <id>|env-set <id> <key> <value>|env-unset <id> <key>|scope-list [id]|scope-set <id> <package|*> <user|-1> <allow|deny>|scope-clear <id>
